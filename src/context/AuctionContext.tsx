...
interface AuctionContextType {
  auctions: Auction[];
  addAuction: (
    auction: Omit<Auction, "id" | "status" | "createdAt" | "updatedAt">
  ) => Promise<void>;
...
export const AuctionProvider = ({ children }: { children: ReactNode }) => {
  ...
  const submitBid = async (
    auctionId: string,
    bidData: Omit<Bid, "id" | "time" | "auctionId" | "auctionTitle">,
    currentLowestBid: number
  ) => {
    ...
    batch.update(auctionRef, {
      currentLowestBid: bidData.amount,
      updatedAt: serverTimestamp(),
    });
  ...
  const getReviewsForUser = useCallback(
    async (userId: string): Promise<Review[]> => {
      ...
      winnerSnapshot.forEach((doc) => {
        ...
        if (data.adminReview)
          allReviews.push({ id: doc.id, ...data.adminReview });
      });

      if (userId === "admin") {
        // A special case to get reviews for the admin
        const allAuctionsSnapshot = await getDocs(collection(db, "auctions"));
        allAuctionsSnapshot.forEach((doc) => {
          const data = doc.data() as AuctionData;
          if (data.winnerReview)
            allReviews.push({ id: doc.id, ...data.winnerReview });
        });
      }
